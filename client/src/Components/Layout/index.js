...
const LayoutComponent = (props, isLogged, style) => {
    ...
    return <Layout>
        <Header style={headerStyle}>
            ...
            <Menu style={{fontSize: '18px'}} onClick={handleMenu} selectedKeys={[current]} mode='horizontal'>
            <Menu.Item key='home'>Home</Menu.Item>
            {isLogged? <Menu.Item key='orders'>Orders</Menu.Item>: null}
            <Menu.Item key='aboutUs'>About Us</Menu.Item>
            </Menu>
            {isLogged? <div style={{display: 'flex', flexDirection: 'row', justifyContent: 'space-around'}}>
                <img src={userPic} alt={userName}/>
                <p style={{fontSize: '13px'}}>{userName}</p>
                </div>: <button onClick={history.push('/login')}>Sign In</button>}
            </Header>
        <Content style={style}>
            {children}
        </Content>
